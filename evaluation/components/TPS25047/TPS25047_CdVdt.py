...
def solve(case: Case) -> Solution:
    ...
    return Solution(sr_v_per_s=sr, cdvdt_nf=cdvdt_nf, iinrush_a=iinrush_a)
...
def run_case(case: Case) -> None:
    ...
    print("\n----------------------------------------")
    print("Solution:")
    print("SR (Slew Rate) = {:.2f} V/s = {:.2f} V/ms".format(sol.sr_v_per_s, sol.sr_v_per_s / 1e3))
    ...
    print("SR from equation: {:.2f} V/s (should be {:.2f} V/s)".format(sr_check, sol.sr_v_per_s))
    print("tR from equation: {:.4f} ms (should be {:.4f} ms)".format(tr_check * 1e3, case.tr_s * 1e3))
